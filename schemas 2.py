...
class AgentMemoryStore:
    ...
    def list_contexts(self, limit: int = 10) -> List[MemoryContext]:
        """List stored contexts with optional limit"""
        contexts = list(self.contexts.values())
        # Sort by created_at descending (most recent first)
        contexts.sort(key=lambda x: x.created_at, reverse=True)
        return contexts[:limit]
    
    def update_status(self, context_id: str, new_status: str) -> bool:
        """Update the status of a context"""
        if context_id in self.contexts:
            self.contexts[context_id].status = new_status
    ...
    def get_contexts_by_status(self, status: str) -> List[MemoryContext]:
        """Get all contexts with a specific status"""
        return [ctx for ctx in self.contexts.values() if ctx.status == status]


class MockBriefRequest(BaseModel):
    """
    Request schema for creating mock campaign briefs
    Used for testing and development
    """
    campaign_type: str = Field(
        default="product_launch",
        description="Type of campaign to generate mock data for"
    )
    company_name: Optional[str] = Field(
        default=None,
        description="Company name to use in mock brief"
    )
    
    @validator('campaign_type')
    def validate_mock_campaign_type(cls, v):
        """Validate campaign type for mock data"""
        allowed_types = [
            'product_launch', 'brand_awareness', 'lead_generation', 
            'conversion', 'retention', 'event_promotion'
        ]
        if v not in allowed_types:
...
class AirtableWebhookPayload(BaseModel):
    """
    Schema for Airtable webhook payloads (for future integration)
    """
    record_id: str = Field(..., description="Airtable record ID")
    table_name: str = Field(..., description="Name of the Airtable table")
    action: str = Field(..., description="Action that triggered webhook (created, updated, deleted)")
    fields: Dict[str, Any] = Field(..., description="Record fields from Airtable")
    timestamp: datetime = Field(default_factory=datetime.now, description="Webhook timestamp")
    
    class Config:
        schema_extra = {
            "example": {
                "record_id": "recABC123XYZ",
                "table_name": "Campaign Briefs",
                "action": "created",
                "fields": {
                    "Campaign ID": "CAMP_2024_002",
                    "Company Name": "Client Corp",
                    "Campaign Type": "brand_awareness",
                    "Objective": "Increase brand recognition in target market"
                },
...
class AgentHealthCheck(BaseModel):
    ...
    agent_role: str = Field(..., description="Current agent role")
    timestamp: datetime = Field(default_factory=datetime.now)
    memory_store_status: str = Field(..., description="Memory store health")
    claude_api_status: str = Field(..., description="Claude API connectivity status")
    total_contexts: int = Field(..., description="Number of stored contexts")
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "memory_store_status": "operational",
...
class BatchProcessRequest(BaseModel):
    """
    Schema for batch processing multiple campaign briefs
    (Future feature for processing multiple campaigns at once)
    """
    briefs: List[CampaignBrief] = Field(..., description="List of campaign briefs to process")
    ...
    def validate_processing_mode(cls, v):
        allowed_modes = ['parallel', 'sequential']
    ...
    class Config:
        schema_extra = {
            "example": {
                "briefs": [
                    {
                        "campaign_id": "CAMP_2024_003",
                        "company_name": "Batch Client 1",
                        "campaign_type": "product_launch"
                    }
                ],
